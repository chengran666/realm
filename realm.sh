...
deploy_realm() {
    mkdir -p /root/realm
    cd /root/realm
    wget -O realm.tar.gz https://cdn.whzxc.com/realm-x86_64-unknown-linux-gnu.tar.gz
...
update_script() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -s /tmp/realm.sh ]; then
            ...
            echo -e "${GREEN}已备份原脚本为 $0.backup${NC}"
            echo -e "${GREEN}请重新运行脚本以应用更新。${NC}"
            exit 0
        else
            echo -e "${RED}更新失败：下载的文件为空${NC}"
            rm -f /tmp/realm.sh
        fi
    else
        echo -e "${RED}更新失败：无法下载新版本${NC}"
    fi
}
